    ...
    <div className={`flex flex-col items-center bottom-0 `}>
      ...
      <div className={`movie-info mt-2 `}>
        {movie.title?<h3 className="text-white text-sm text-center font-semibold">{movie.title.split(":")[0]} </h3>
                    :<h3 className="text-white text-sm text-center font-semibold">{movie.name.split(":")[0]} <br></br>{movie.character? <span className='text-[10px]'>( {movie.character.split(" (")[0]} )</span>:null}</h3>
        }
        {season? <p className="text-white text-xs text-center">{movie.episode_count} Episodes</p>:null}
      </div>
    </div>
  );
};
